...
starting_time = '2022-08-18 17:27:00' # any select time in this format
format_data = "20%y-%m-%d %H:%M:%S" # time format
box_hz = 20 # 20 Hz is the sampling rate of the paro
sample_rate = 1 # sample/min, user desired sampling rate of the plot
sample_time = 120 # miuntes, user desired time duration of the plot
alpha = 0.03 # smoothing factor, 1 > alpha > 0
event_name = "C5 TOUCHGO" # Name of the event, show n the title of the graph
...
def data_download():
    ...
    start_time = datetime.fromisoformat("2023-05-09T03:42:45")
    end_time = datetime.fromisoformat("2023-05-09T03:52:45")
    ...
    idb_deviceid_query = \
        ...
        'schema.tagValues('\
        'bucket: "' + "paros-live-datastream" + '",'\
        'predicate: (r) => r["_measurement"] == "' + "paros1" + '",'\
...
def categorize_paros_datafile():
    ...
    ## categorize 6 barometers in two boxes into seperate lists with the corresponding sampe rate
...
def exp_smoothing_from_raw_data(time_matched_index, parometer_model_data):
    ...
    interval = sample_rate*20*60

    for i in range(1, sample_time*60*20, interval):
        a = time_matched_index+i
        b = int((i-1)/interval)
        x_t = parometer_model_data[a]
        raw.append(float(x_t[3]))
        predict = str(alpha * float(x_t[3]) + (1 - alpha) *float(exp_smooth[b]))
...
def bpf_pressure(pressure):
    n = len(pressure) # number of data points
    timestep = 1/20 # time between data points (in seconds)
    ...
    # apply FFT
    pressure_fft = np.fft.fft(pressure_np)

    # apply bandpass filter
    min_freq = 0.5 # Hz
    max_freq = 3 # Hz
    pressure_fft_filtered = pressure_fft.copy() # create copy of FFT data
    pressure_fft_filtered[(freq < min_freq) | (freq > max_freq)] = 0 # set frequencies outside bandpass to zero
...
def gen_fft_ac_graph_single(raw_1, exp_smooth_1, starting_date):
    
    fft_window_size = 2**A
    x_matrix = []
    y1 = fft_ac(raw_1, exp_smooth_1)
    number_of_windows = len(y1)
    for i in range(number_of_windows):
        x_matrix.append(i/1200)
    x = x_matrix
    ...
    plt.plot(x, y1, label = title_1, linewidth=1, color = 'magenta')
    # plt.text(0.1, 0.1, device_name_6, ha="center", va="center", transform=plt.gca().transAxes, fontsize = 18, fontweight = 'bold')
    startnum = 1000*(starting_date.hour*60 + starting_date.minute)
    plt.legend(fontsize=12, bbox_to_anchor=[0.5, 1], loc='center', ncol=3, framealpha=1)
    plt.xticks(fontsize=16)
...
def gen_bpf_graph(raw_1, raw_2, raw_3, raw_4, raw_5, raw_6, starting_date):
    
    fft_window_size = 2**A
    x_matrix = []
    y1, y2, y3, y4, y5, y6 = bpf_pressure(raw_1), bpf_pressure(raw_2),bpf_pressure(raw_3),bpf_pressure(raw_4),bpf_pressure(raw_5),bpf_pressure(raw_6)
    number_of_windows = len(y1)
    for i in range(number_of_windows):
        x_matrix.append(i/1200)
    x = x_matrix
    fig_main = plt.figure(figsize=(15, 10))
    st = "%s/%s/%s %s:%s"%(starting_date.year, starting_date.month, starting_date.day, starting_date.hour, starting_date.minute)
    fig_main.suptitle("%s -- Pressure (bandpass filtered to 0.5Hz-3Hz) \n (starting time is %s:00)"%( event_name, st), fontsize=20, fontweight = 'bold')
    fig_main.supxlabel("Timestamp (UTC)", fontsize=20,fontweight = 'bold')
    fig_main.supylabel("Pressure (hPa)", fontsize=20, fontweight = 'bold')
    # title_1 = 'Sum of the FFT in the window of %s samples shift by %s'%(fft_window_size,2**B)
    device_name_1 = "paro1-141920"
    ...
    plt.subplot(6, 1, 1)
    plt.plot(x, y1, linewidth=1, color = 'blue')
    plt.text(0.1, 0.1, device_name_1, ha="center", va="center", transform=plt.gca().transAxes, fontsize = 18, fontweight = 'bold')
    plt.xticks([])
    # plt.legend(fontsize=12, bbox_to_anchor=[0.5, 1], loc='center', ncol=3, facecolor='white', framealpha=1)
    ...
    plt.subplot(6, 1, 2)
    plt.plot(x, y2, linewidth=1, color = 'green')
    plt.text(0.1, 0.1, device_name_2, ha="center", va="center", transform=plt.gca().transAxes, fontsize = 18, fontweight = 'bold')
    # plt.legend(fontsize=12, bbox_to_anchor=[0.5, 1], loc='center', ncol=3, facecolor='white', framealpha=1)
    plt.xticks([])  
    ...
    plt.subplot(6, 1, 4)
    plt.plot(x, y4, linewidth=1, color = 'brown')
    plt.text(0.1, 0.1, device_name_4, ha="center", va="center", transform=plt.gca().transAxes, fontsize = 18, fontweight = 'bold')
    plt.xticks([])  
    # plt.legend(fontsize=12, bbox_to_anchor=[0.5, 1], loc='center', ncol=3, facecolor='white', framealpha=1)

    plt.subplot(6, 1, 5)
    plt.plot(x, y5, linewidth=1, color = 'red')
    plt.text(0.1, 0.1, device_name_5, ha="center", va="center", transform=plt.gca().transAxes, fontsize = 18, fontweight = 'bold')
    plt.xticks([])  
    # plt.legend(fontsize=12, bbox_to_anchor=[0.5, 1], loc='center', ncol=3, facecolor='white', framealpha=1)
    ...
    plt.subplot(6, 1, 6)
    plt.plot(x, y6, linewidth=1, color = 'magenta')
    plt.text(0.1, 0.1, device_name_6, ha="center", va="center", transform=plt.gca().transAxes, fontsize = 18, fontweight = 'bold')
    startnum = 1000*(starting_date.hour*60 + starting_date.minute)
    # plt.legend(fontsize=12, bbox_to_anchor=[0.5, 1], loc='center', ncol=3, framealpha=1)
    plt.xticks(fontsize=16)
...
def main():
    ...
    # generate power fft over time for "all paros"
    ...
    # generate power fft over time for "single barometer"
    # gen_fft_ac_graph_single(raw_141905, exp_smooth_141905, starting_date)

    # generate power fft over time with bandpass filter for "all paros"
    # gen_bpf_graph(raw_141920, raw_142180, raw_141906, raw_142176, raw_141905, raw_141931, starting_date)
